...
(* $Id: xml.ml 1654 2012-05-22 17:21:28Z maarek $ *)
...
module M_unexp = Map.Make
    (struct
      type t = unexpanded_name
      let compare
          { prefix = prefix_1; local_name = local_name_1 }
          { prefix = prefix_2; local_name = local_name_2 } =
        match Lib.opt_compare Stringdata.comp prefix_1 prefix_2 with
        | 0 -> Stringdata.comp local_name_1 local_name_2
        | n -> n
    end)

type unexpanded_attributes = Stringdata.t M_unexp.t

module M_exp = Map.Make
    (struct
      type t = expanded_name
      let compare
          { uri = uri_1; name = name_1 }
          { uri = uri_2; name = name_2 } =
        match Stringdata.comp uri_1 uri_2 with
        | 0 -> Stringdata.comp name_1 name_2
        | n -> n
    end)
